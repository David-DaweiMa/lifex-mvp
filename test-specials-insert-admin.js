...
async function testSpecialsInsertAdmin() {
  console.log('🔐 使用管理员权限测试Specials数据插入\n');
  ...
  try {
    ...
    const testData = {
      ...
      title: 'Admin Test Special - ' + new Date().toISOString(),
      description: 'This is a test special inserted with admin privileges',
      ...
      claimed: 0,
      max_claims: 50,
      tags: ['test', 'admin', 'validation'],
      terms_conditions: 'This is a test special inserted with admin privileges for validation purposes only.'
    };
    ...
    if (insertResult.status === 201) {
      ...
      if (verifyResult.data && verifyResult.data.length > 0) {
        ...
        console.log(`    - 条款: ${insertedData.terms_conditions}`);
      }
    } else {
  ...
  console.log('\n✅ 管理员测试完成！');
  console.log('\n💡 如果测试成功，说明SQL脚本的字段映射是正确的');
  console.log('💡 您可以在Supabase Dashboard中直接执行 add-specials-data-final.sql');
}

testSpecialsInsertAdmin().catch(error => {
