-- 为specials表添加20条测试数据的简化版本
-- 使用现有的商家数据，避免创建新商家

-- 首先检查现有的商家数据
SELECT '现有商家数量:' as info, COUNT(*) as count FROM public.businesses;

-- 如果商家数量不足，创建一个测试商家
INSERT INTO public.businesses (id, name, description, logo_url, category, is_verified, is_active, created_at, updated_at)
SELECT 
  gen_random_uuid(),
  'Test Business for Specials',
  'Test business for specials data',
  ...
  NOW(),
  NOW()
WHERE NOT EXISTS (SELECT 1 FROM public.businesses WHERE name = 'Test Business for Specials');

-- 获取一个商家ID用于关联
WITH test_business AS (
  SELECT id FROM public.businesses WHERE name = 'Test Business for Specials' LIMIT 1
)
