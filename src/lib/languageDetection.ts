 ...
 */
export const detectMultipleLanguages = (text: string): SupportedLanguage[] => {
  if (!text || typeof text !== 'string') {
    return ['en'];
  }
...
/**
 * Get primary language (most likely based on character count)
 * @param text - The text to analyze
 * @returns Primary language code
 */
export const getPrimaryLanguage = (text: string): SupportedLanguage => {
  ...
  // For mixed language text, prioritize based on character count
  const charCounts: Record<SupportedLanguage, number> = {
    en: 0, zh: 0, ja: 0, ko: 0, es: 0, fr: 0, de: 0
  };

  for (const [lang, pattern] of Object.entries(LANGUAGE_PATTERNS)) {
    const matches = text.match(pattern);
    if (matches) {
      charCounts[lang as SupportedLanguage] = matches.join('').length;
    }
  }

  // Count English characters (non-special characters)
  const englishChars = text.replace(/[^a-zA-Z\s]/g, '').length;
  charCounts.en = englishChars;
